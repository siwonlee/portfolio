<script setup>
import { ref } from "vue";

defineProps({
  msg: String,
});

const count = ref(0);
</script>

<template>
  <h1 class="text-4xl font-bold mt-0 mb-6">Siwon's Web and Mobile Portfolio</h1>

  <!-- Project One -->

  <div class="flex mt-3">
    <div class="w-2/3" align="center">
      <a href="#">
        <img
          class="img-fluid rounded mb-3 mb-md-0 shadow-lg"
          src="../assets/img/1.jpg"
          alt=""
        />
      </a>
    </div>
    <div class="w-1/3 text-left ml-3">
      <div class="text-2xl font-bold mb-3">MD(Maryland) APL Management</div>
      <p class="text-gray-700 text-base mb-3">
        The site has been developed for the following purpose(s).
      </p>

      <ul class="list-disc list-outside ml-3 mb-3">
        <li class="">
          Approve(or Deny) the store collected or vendor uploaded upcs
        </li>
        <li>Generate the Maryland APL file for WOW to send it to SOAR</li>
      </ul>

      <p class="mb-3">Target users : Nutritionist, Vendor Staff</p>
      <a
        class="btn btn-primary"
        target="_blank"
        href="https://apl.mdwic.org/upc_verify_new"
        >View Project</a
      >
      <a
        class="btn btn-info"
        target="_blank"
        href="https://apl.mdwic.org/upc_verify_new/manual_UPC_Verification.pdf"
        >Manual</a
      >
      <a
        class="btn btn-danger"
        target="_blank"
        href="https://apl.mdwic.org/tutorial/index.html"
        >Video</a
      >
    </div>
  </div>

  <!-- /.row -->

  <hr />

  <!-- Project Two -->
  <div class="flex mt-3">
    <div class="w-2/3" align="center">
      <a href="#">
        <img
          class="img-fluid rounded mb-3 mb-md-0 shadow-lg"
          src="../assets/img/1.jpg"
          alt=""
        />
      </a>
    </div>
    <div class="w-1/3 text-left ml-3">
      <div class="text-2xl font-bold mb-3">
        VI(Virgin Islands) APL Management
      </div>
      <p class="mb-3">
        The site has been developed for the following purpose(s).
      </p>
      <ul class="list-disc list-outside ml-3 mb-3">
        <li>Approve(or Deny) the store collected upcs</li>
        <li>Generate the Virgin Islands APL file for WOW to send it to SOAR</li>
      </ul>
      <p class="mb-3">Target users : Nutritionist, Vendor Staff</p>
      <a
        class="btn btn-primary"
        target="_blank"
        href="https://apl.mdwic.org/upc_verify_vi_new/"
        >View Project</a
      >
      <a
        class="btn btn-info"
        target="_blank"
        href="https://apl.mdwic.org/upc_verify_new/manual_UPC_Verification.pdf"
        >Manual</a
      >
      <a
        class="btn btn-danger"
        target="_blank"
        href="https://apl.mdwic.org/tutorial/index.html"
        >Video</a
      >
    </div>
  </div>
  <!-- /.row -->

  <hr />

  <!-- Project Three -->
  <div class="flex mt-3">
    <div class="w-2/3" align="center">
      <a href="#">
        <img
          class="img-fluid rounded mb-3 mb-md-0 shadow-lg"
          src="../assets/img/3.png"
          alt=""
        />
      </a>
    </div>
    <div class="w-1/3 text-left ml-3">
      <div class="text-2xl font-bold mb-3">UPC Collector(for Maryland)</div>
      <p class="mb-3">
        <span class="text-base font-bold"
          >The window tablet based application</span
        >
        to collect upcs at the stores with the attached barcode scanner and to
        generate an Excel and a SQL file for uploading the collected upcs to the
        APL Management site.
      </p>

      <p class="mb-3">Target users : Nutritionist, Vendor Staff</p>

      <a
        class="btn btn-info"
        target="_blank"
        href="https://apl.mdwic.org/UPC_Collector_VI/manual_UPC_Collector_VI_03282016.pdf
"
        >Manual</a
      >
      <a
        class="btn btn-danger"
        target="_blank"
        href="https://apl.mdwic.org/tutorial/index_collector.html
"
        >Video</a
      >
      <a
        class="btn btn-warning"
        target="_blank"
        href="https://apl.mdwic.org/update/VI/old/UPC_Collector_VI_03012017.rar
"
        >Download</a
      >
    </div>
  </div>
  <!-- /.row -->

  <hr />

  <!-- Project Four -->
  <div class="flex mt-3">
    <div class="w-2/3" align="center">
      <a href="#">
        <img
          class="img-fluid rounded mb-3 mb-md-0 shadow-lg"
          src="../assets/img/4.png"
          alt=""
        />
      </a>
    </div>
    <div class="w-1/3 text-left ml-3">
      <div class="text-2xl font-bold mb-3">
        UPC Collector(for Virgin Islands)
      </div>
      <p class="mb-3">
        <span class="text-base font-bold"
          >The window tablet based application</span
        >
        to collect upcs at the stores with the attached barcode scanner and to
        generate an Excel and a SQL file for uploading the collected upcs to the
        APL Management site.
      </p>

      <p class="mb-3">Target users : Nutritionist, Vendor Staff</p>
      <a
        class="btn btn-info"
        target="_blank"
        href="https://apl.mdwic.org/UPC_Collector_VI/manual_UPC_Collector_VI_03282016.pdf
"
        >Manual</a
      >
      <a
        class="btn btn-danger"
        target="_blank"
        href="https://apl.mdwic.org/tutorial/index_collector.html
"
        >Video</a
      >
      <a
        class="btn btn-warning"
        target="_blank"
        href="https://apl.mdwic.org/update/VI/old/UPC_Collector_VI_03012017.rar
"
        >Download</a
      >
    </div>
  </div>
  <!-- /.row -->

  <hr />

  <!-- Project Four -->
  <div class="flex mt-3">
    <div class="w-2/3" align="center">
      <a href="#">
        <img
          class="img-fluid rounded mb-3 mb-md-0 shadow-lg"
          src="../assets/img/5.png"
          alt=""
          align="center"
        />
      </a>
    </div>
    <div class="w-1/3 text-left ml-3">
      <div class="text-2xl font-bold mb-3">Vendor UPC Upload</div>
      <p class="mb-3">
        The site has been developed for the following purpose(s).
      </p>
      <ul class="list-disc list-outside ml-3 mb-3">
        <li>
          Upload upcs for the State’s approval by using the Single or File
          upload features
        </li>
      </ul>
      <p class="mb-3">
        Target users : Vendors, manufacturers, suppliers,participants
      </p>
      <a
        class="btn btn-primary"
        target="_blank"
        href="https://apl.mdwic.org/upc_upload_new
"
        >View Project</a
      >
      <a
        class="btn btn-info"
        target="_blank"
        href="https://apl.mdwic.org/upc_upload_new/upc_upload_manual.pdf
"
        >Manual</a
      >
      <a
        class="btn btn-danger"
        target="_blank"
        href="https://apl.mdwic.org/tutorial/index_vendor.html
"
        >Video</a
      >
    </div>
  </div>
  <!-- /.row -->

  <hr />
  <!-- Project Four -->
  <div class="flex mt-3">
    <div class="w-2/3" align="center">
      <a href="#">
        <img
          class="img-fluid rounded mb-3 mb-md-0 shadow-lg"
          src="../assets/img/6.png"
          alt=""
        />
      </a>
    </div>
    <div class="w-1/3 text-left ml-3">
      <div class="text-2xl font-bold mb-3">Product Submission</div>
      <p class="mb-3">
        The site has been developed for the following purpose(s).
      </p>
      <ul class="list-disc list-outside ml-3 mb-3">
        <li>
          Upload upcs for the State’s approval for the annual or biannual
          revision of the Authorized Foods List
        </li>
      </ul>
      <p class="mb-3">Target users : manufacturers and suppliers</p>
      <a
        class="btn btn-primary"
        target="_blank"
        href="https://apl.mdwic.org/upc_submit_2018
"
        >View Project</a
      >
      <a
        class="btn btn-info"
        target="_blank"
        href="https://apl.mdwic.org/upc_submit_template/manual_UPC_Submit_01292018.pdf 
"
        >Manual</a
      >
      <a
        class="btn btn-danger"
        target="_blank"
        href="https://apl.mdwic.org/tutorial/index_manufacturer.html
"
        >Video</a
      >
    </div>
  </div>
  <!-- /.row -->

  <hr />
  <!-- Project Four -->
  <div class="flex mt-3">
    <div class="w-2/3" align="center">
      <a href="#">
        <img
          class="img-fluid rounded mb-3 mb-md-0 shadow-lg"
          src="../assets/img/7.png"
          alt=""
          width="250px"
        />
      </a>
    </div>
    <div class="w-1/3 text-left ml-3">
      <div class="text-2xl font-bold mb-3">MD APL Check</div>
      <p class="mb-3">
        The site has been developed for the following purpose(s).
      </p>
      <ul class="list-disc list-outside ml-3 mb-3">
        <li>Check an upc whether it is a Maryland WIC approved item or not</li>
      </ul>
      <p class="mb-3">Target users : participants, vendors</p>
      <a
        class="btn btn-primary"
        target="_blank"
        href="https://apl.mdwic.org/upc_web/
"
        >View Project</a
      >
    </div>
  </div>
  <!-- /.row -->

  <hr />
  <!-- Project Four -->
  <div class="flex mt-3">
    <div class="w-2/3" align="center">
      <a href="#">
        <img
          class="img-fluid rounded mb-3 mb-md-0 shadow-lg"
          src="../assets/img/8.png"
          alt=""
          width="250px"
        />
      </a>
    </div>
    <div class="w-1/3 text-left ml-3">
      <div class="text-2xl font-bold mb-3">MD APL Check App(for Android)</div>
      <p class="mb-3">
        The Android phone app to check an upc whether it is a Maryland WIC
        approved item or not
      </p>

      <p class="mb-3">Target users : participants, vendors</p>

      <!--	  <a class="btn btn-warning" target='_blank' href="http://apl.mdwic.org/upc_app/upc_checker.apk">download</a>-->
      <a
        class="btn btn-warning"
        target="_blank"
        href="https://play.google.com/store/apps/details?id=io.ionic.starter.aplchecker"
        >download</a
      >

      <a
        class="btn btn-danger"
        target="_blank"
        href="https://www.youtube.com/watch?v=IvfvxT6XTso"
        >Video</a
      >
    </div>
  </div>
  <!-- /.row -->

  <hr />
  <!-- Project Four -->
  <div class="flex mt-3">
    <div class="w-2/3" align="center">
      <a href="#">
        <img
          class="img-fluid rounded mb-3 mb-md-0 shadow-lg"
          src="../assets/img/8.png"
          alt=""
          width="250px"
        />
      </a>
    </div>
    <div class="w-1/3 text-left ml-3">
      <div class="text-2xl font-bold mb-3">MD APL Check App(for iOS)</div>
      <p class="mb-3">
        The iPhone app to check an upc whether it is a Maryland WIC approved
        item or not
      </p>

      <p class="mb-3">Target users : participants, vendors</p>

      <a
        class="btn btn-warning"
        target="_blank"
        href="https://itunes.apple.com/us/app/md-wic-apl-checker/id1455361859?mt=8"
        >download</a
      >
      <a
        class="btn btn-danger"
        target="_blank"
        href="https://www.youtube.com/watch?v=IvfvxT6XTso"
        >Video</a
      >
    </div>
  </div>
  <!-- /.row -->

  <hr />
  <!-- Project 9 -->
  <div class="flex mt-3">
    <div class="w-2/3" align="center">
      <a href="#">
        <img
          class="img-fluid rounded mb-3 mb-md-0 shadow-lg"
          src="../assets/img/9.png"
          alt=""
        />
      </a>
    </div>
    <div class="w-1/3 text-left ml-3">
      <div class="text-2xl font-bold mb-3">qWICk</div>
      <p class="mb-3">
        The site has been developed for the following purpose(s).
      </p>
      <ul class="list-disc list-outside ml-3 mb-3">
        <li>
          Provide online Vehicle reservation system, useful links and the org
          chart
        </li>
      </ul>

      <p class="mb-3">Target users : State WIC Staff</p>
      <a
        class="btn btn-primary"
        target="_blank"
        href="https://apl.mdwic.org/qwick_new/"
        >View Project</a
      >
    </div>
  </div>
  <!-- /.row -->

  <hr />
  <!-- Project 10 -->
  <div class="flex mt-3">
    <div class="w-2/3" align="center">
      <a href="#">
        <img
          class="img-fluid rounded mb-3 mb-md-0 shadow-lg"
          src="../assets/img/10.png"
          alt=""
        />
      </a>
    </div>
    <div class="w-1/3 text-left ml-3">
      <div class="text-2xl font-bold mb-3">Breastfeeding Support Role Site</div>
      <p class="mb-3">
        The site has been developed for the following purpose(s).
      </p>
      <ul class="list-disc list-outside ml-3 mb-3">
        <li>
          Assign the breastfeeding roles to the staff of the local agencies
        </li>
      </ul>

      <p class="mb-3">
        Target users : Breastfeeding coordinators of WIC Local agencies
      </p>
      <a
        class="btn btn-primary"
        target="_blank"
        href="https://apl.mdwic.org/bf_roles
"
        >View Project</a
      >
      <a
        class="btn btn-info"
        target="_blank"
        href="https://apl.mdwic.org/bf_roles/bf_roles_20180713.pdf
"
        >Manual</a
      >
      <a
        class="btn btn-danger"
        target="_blank"
        href="https://apl.mdwic.org/tutorial/index_bf.html
"
        >Video</a
      >
    </div>
  </div>
  <!-- /.row -->

  <hr />

  <!-- Project 10 -->
  <div class="flex mt-3">
    <div class="w-2/3" align="center">
      <a href="#">
        <img
          class="img-fluid rounded mb-3 mb-md-0 shadow-lg"
          src="../assets/img/tutorial_channel.png"
          alt=""
        />
      </a>
    </div>
    <div class="w-1/3 text-left ml-3">
      <div class="text-2xl font-bold mb-3">Tutorial Channel</div>
      <p class="mb-3">
        The site contains the tutorial videos of the all the web and mobile
        projects.
      </p>

      <p class="mb-3">
        Target users : All the users of the apps and the programs
      </p>
      <a
        class="btn btn-primary"
        target="_blank"
        href="https://apl.mdwic.org/tutorial/index.php"
        >View Project</a
      >
    </div>
  </div>
  <!-- /.row -->

  <!-- Project 10 -->
  <div class="flex mt-3">
    <div class="w-2/3" align="center">
      <a href="#">
        <img
          class="img-fluid rounded mb-3 mb-md-0 shadow-lg"
          src="../assets/img/vendor_audit_img.png"
          alt=""
        />
      </a>
    </div>
    <div class="w-1/3 text-left ml-3">
      <div class="text-2xl font-bold mb-3">Inventory Audit App</div>
      <p class="mb-3">
        The app is used to collect and count inventory audit items on site. The
        form is being generated and will be used at the Inventory Audit
        Management Site.
      </p>

      <p class="mb-3">Target users : Vendor Unit</p>

      <a
        class="btn btn-info"
        target="_blank"
        href="https://apl.mdwic.org/vendor_audit/manual_inventory_audit.pdf"
        >Manual</a
      >
      <a
        class="btn btn-danger"
        target="_blank"
        href="https://apl.mdwic.org/tutorial/inventory_audit.php"
        >Video</a
      >
    </div>
  </div>
  <!-- /.row -->

  <!-- Project 10 -->
  <div class="flex mt-3">
    <div class="w-2/3" align="center">
      <a href="#">
        <img
          class="img-fluid rounded mb-3 mb-md-0 shadow-lg"
          src="../assets/img/inventory_audit_management.png"
          alt=""
        />
      </a>
    </div>
    <div class="w-1/3 text-left ml-3">
      <div class="text-2xl font-bold mb-3">Inventory Audit Management</div>
      <p class="mb-3">you can manage all the inventory audit procedures.</p>

      <p class="mb-3">Target users : Vendor Unit</p>

      <a
        class="btn btn-primary"
        target="_blank"
        href="https://apl.mdwic.org/vendor_audit"
        >View Project</a
      >
    </div>
  </div>
  <!-- /.row -->

  <!-- Project 10 -->
  <div class="flex mt-3">
    <div class="w-2/3" align="center">
      <a href="#">
        <img
          class="img-fluid rounded mb-3 mb-md-0 shadow-lg"
          src="../assets/img/data_analysis.png"
          alt=""
        />
      </a>
    </div>
    <div class="w-1/3 text-left ml-3">
      <div class="text-2xl font-bold mb-3">Data Analysis</div>
      <p class="mb-3">
        The site generates in-depth reports of APL and transaction data.
      </p>

      <p class="mb-3">Target users : WIC Staff</p>

      <a
        class="btn btn-primary"
        target="_blank"
        href="http://siwon.ddns.net/stat"
        >View Project</a
      >
      <a
        class="btn btn-info"
        target="_blank"
        href="http://siwon.ddns.net/stat/manual_Data_Analysis_20200211.pdf"
        >Manual</a
      >
      <a
        class="btn btn-danger"
        target="_blank"
        href="https://www.youtube.com/watch?v=bTCTqbUcFWk&t=40s"
        >Video</a
      >
    </div>
  </div>
  <!-- /.row -->
</template>

<style scoped>
.btn {
  @apply py-2 px-4 rounded mr-1;
}
.btn-primary {
  @apply bg-blue-500 text-white;
}
.btn-primary:hover {
  @apply bg-blue-700;
}

.btn-info {
  @apply bg-green-500 text-white;
}
.btn-info:hover {
  @apply bg-green-700;
}

.btn-danger {
  @apply bg-red-500 text-white;
}
.btn-danger:hover {
  @apply bg-red-700;
}

.btn-warning {
  @apply bg-yellow-500 text-white;
}
.btn-warning:hover {
  @apply bg-yellow-700;
}
</style>
